...
if __name__ == '__main__':
    ...
    if not policyFolder.exists():
        print(f'Policy folder {policyFolder} does not exist')
        sys.exit(1)
    ...
    if not policyFolderRoot.exists():
        print(f'Policy folder root {policyFolderRoot} does not exist')
        sys.exit(1)

    if not str(policyFolder).startswith(str(policyFolderRoot)):
        print(f'Policy folder {policyFolder} is not under Policy folder root {policyFolderRoot}')
        sys.exit(1)

    all_policy_entries = []

    for p in policyFolderRoot.glob(POLICY_FILE_PATTERN):
    ...
    for csvfilename in glob.glob(POLICY_FILE_PATTERN):
        ...
        with open(csvfilename, 'r') as csvfile:
            ...
            for row in csv_list:
                print(f"### {row}")
                if len(row) != 3:
                    print(f"file: {csvfilename} - incorrect number of columns in row: {row}")
                    continue
                root_folder = row[0]
                try:
                    ...
                    print(f'Error {err} WHEN attempting to extract float numbers from HK rule. This line will be skipped: {row}')
                    continue
                
                other_more_specific_root_folders_list = [r[0] for r in all_policy_entries if r[0] != root_folder and r[0].startswith(root_folder)]
        ...
        print(f"File {csvfilename} - Elapsed time in seconds: {time_end_csvfile - time_init_csvfile}")
        metrics_add(f'{metrics_id_prefix}._elapsedTimeAggregated_All', str(time_end_csvfile - time_init_csvfile))
        metrics_send_pickle()

    time_end_global = time.time()

    print(f"Global Elapsed time in seconds: {time_end_global - time_init_global}")
